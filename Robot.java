...
public class Robot
{
    ...
    public Robot(int theX, int theY)
    {
        ...
        direction=0;//at start stands the roboter toward the top of the window (north)
    ...
    // getDistanceFromStart() returns the distance the robot is from the starting point.
    public double getDistanceFromStart(){
     /* if(startX==actualX && startY==actualY){
          return 0;
        } 
        else{
             Point current = new Point(actualX, actualY);
             return original.distance(current);
        }*/
    double distance=0;
    double diffX=actualX-startX;
    double diffY= actualY-startY;
